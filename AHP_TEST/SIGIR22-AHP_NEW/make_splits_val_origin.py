        ...
        for idx in total_idx :
            if idx in ground_idx:
                if idx in ground_valid_idx:
                    ground_valid_data.append(HE[idx]) # 10
                else:
                    ground_train_data.append(HE[idx]) # 50
            else :
                pred_data.append(HE[idx]) # 40
                
        valid_only_num = int(0.25*len(pred_data)) # 10
        train_only_num = int(0.25*len(pred_data)) # 10
        test_num = len(pred_data) - (valid_only_num + train_only_num) # 20
        
        random.shuffle(pred_data)
        train_only_data = pred_data[:train_only_num] # 10
        valid_only_data = pred_data[train_only_num:-test_num] # 10
        test_data = pred_data[-test_num:] # 20
        # negatives        
