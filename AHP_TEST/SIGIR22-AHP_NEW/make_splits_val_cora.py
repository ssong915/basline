...
def split_dataset(dataset):
    ...
    base_num = len(base_cover) # 10
    
    for split in range(5):
        # ground 60%, train 10(+50)%, validation 10(+10)%, test 20%
        ground_num = int(0.6*len(HE)) - base_num # 50
        total_idx = list(range(len(HE))) # 100
        ground_idx = list(set(total_idx)-set(base_cover)) # 90
        ground_idx = random.sample(ground_idx, ground_num) # 50  
        ground_num += base_num # 50+10
        ground_idx += base_cover # 50+10+10
        ground_valid_num = ground_num//6 # 8
        ground_valid_idx = random.sample(ground_idx, ground_valid_num)
        ground_train_num = ground_num - ground_valid_num # 42
        
        ground_train_data = [] # 42
        ground_valid_data = [] # 8
        pred_data = []
