...
def train(args):
    ...
    f_log = open(f"/home/dake/workspace/AHP_TEST/SIGIR22-AHP_NEW/logs/{args.folder_name}_train.log", "w")
    ...
    for j in tqdm(range(args.exp_num)):  
        ...
        data_dict = torch.load(f'/home/dake/workspace/AHP_TEST/SIGIR22-AHP/data/splits/{args.dataset_name}split{j}.pt')
       ...
       # model = models.HGNN(in_ch= args.input_dim, n_hid=args.dim_vertex, dropout=0.5)
        model.to(device)        
        Aggregator = None    
        ...
        for epoch in tqdm(range(args.epochs), leave=False):            
            ...
            while True :
                ...
                d_loss, g_loss, train_pred, train_label, neg_hedge_info = model_train(args, g, model, Aggregator, Generator, optim_D, optim_G, pos_hedges, pos_labels, train_pred, train_label, device, epoch)
                pos_size = sum(len(pos_hedges[i]) for i in range(len(pos_hedges)))/ len(pos_hedges)
                pos_size_total += pos_size
                neg_size = sum(len(neg_hedge_info[i]) for i in range(len(neg_hedge_info)))/ len(neg_hedge_info)
                neg_size_total += neg_size
                neg_hedge_list.extend(neg_hedge_info)
                #------------------------duplication check------------------------------#
                #pos_hedges_list = [p_tensor.tolist() for p_tensor in total_hedge_data]
                #total_len = len(total_hedge_data) + len(neg_hedge_info)
                #duplicates = []
                #for target_e in neg_hedge_info :
                    #if target_e in total_hedge_data :
                        #duplicates.append(target_e)
                #if len(duplicates) != 0 :
                    #print("(ALL)The number of duplicates : ", len(duplicates))
                    #print("(ALL)The ratio of duplicates : ", len(duplicates)/total_len)
                #------------------------duplication check------------------------------#
                ...
                if is_last :
                    break
            neg_size_dist = utils.gen_size_dist(neg_hedge_list)
            print("negative_distribution = ", neg_size_dist)
            print("average positive hyperedge size =", (pos_size_total/count))
            print("average negative hyperedge size =", (neg_size_total/count))
            ...
            #------------------------duplication check------------------------------#
            pos_hedges_list = val_batchloader_pos.hyperedges
            total_len = len(pos_hedges_list) + len(neg_hedge_list)

            duplicates = []
            for target_e in neg_hedge_list :
                if target_e in pos_hedges_list :
                    duplicates.append(target_e)

            if len(duplicates) != 0 :
                print("(validation)The number of duplicates : ", len(duplicates))
                print("(validation)The ratio of duplicates : ", len(duplicates)/total_len)
            #------------------------duplication check------------------------------#

            val_pred_pos, total_label_pos = model_eval(args, val_batchloader_pos, g, model, Aggregator,device)
            val_pred_sns, total_label_sns = model_eval(args, val_batchloader_sns, g, model, Aggregator,device)
            ...
            val_pred_mns, total_label_mns = model_eval(args, val_batchloader_mns, g, model, Aggregator,device)
            ...
            val_pred_cns, total_label_cns = model_eval(args, val_batchloader_cns, g, model, Aggregator,device)
            ...
            print("best roc : ", best_roc)
            print("summation result = ", (auc_roc_sns+auc_roc_mns+auc_roc_cns)/3)
            if best_roc < (auc_roc_sns+auc_roc_mns+auc_roc_cns)/3:
                best_roc = (auc_roc_sns+auc_roc_mns+auc_roc_cns)/3
                best_epoch=epoch
                print("best_epoch : ", best_epoch)
                torch.save(model.state_dict(), f"/home/dake/workspace/AHP_TEST/SIGIR22-AHP/data/checkpoints/{args.folder_name}/model_{j}.pkt")
                torch.save(Aggregator.state_dict(), f"/home/dake/workspace/AHP_TEST/SIGIR22-AHP/data/checkpoints/{args.folder_name}/Aggregator_{j}.pkt")
                torch.save(Generator.state_dict(), f"/home/dake/workspace/AHP_TEST/SIGIR22-AHP/data/checkpoints/{args.folder_name}/Generator_{j}.pkt")
        
        with open(f"/home/dake/workspace/AHP_TEST/SIGIR22-AHP/data/checkpoints/{args.folder_name}/best_epochs.logs", "a") as e_log:  
            e_log.write(f"exp {j} best epochs: {best_epoch}\n")

        test_batchloader_pos = load_test(data_dict, args.bs, device, label="pos")
        #------------------------duplication check------------------------------#
        pos_hedges_list = test_batchloader_pos.hyperedges
        total_len = len(pos_hedges_list) + len(neg_hedge_list)
        ...
        if len(duplicates) != 0 :
            print("(test)The number of duplicates : ", len(duplicates))
            print("(test)The ratio of duplicates : ", len(duplicates)/total_len)
        #------------------------duplication check------------------------------#
...
def test(args, j): 
    args.folder_name = 'exp0'  
    args.checkpoint = f"/home/dake/workspace/AHP_TEST/SIGIR22-AHP/data/checkpoints/{args.folder_name}"
    f_log = open(f"/home/dake/workspace/AHP_TEST/SIGIR22-AHP/logs/{args.folder_name}_results.log", "w")    
    f_log.write(f"{args}\n")    
    ...
    with torch.no_grad():
        test_pred_pos, total_label_pos = model_eval(args, test_batchloader_pos, g, model, Aggregator, device)
        test_pred_sns, total_label_sns = model_eval(args, test_batchloader_sns, g, model, Aggregator, device)
        ...
        test_pred_mns, total_label_mns = model_eval(args, test_batchloader_mns, g, model, Aggregator, device)
        ...
        test_pred_cns, total_label_cns = model_eval(args, test_batchloader_cns, g, model, Aggregator, device)
