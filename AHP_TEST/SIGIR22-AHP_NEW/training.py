...
def model_train(args, g, model, Aggregator, Generator, optim_D, optim_G, pos_hedges, pos_labels, train_pred, train_label, device, epoch, snapshot_node_index):
    ...
    if args.model == "hnhn":
        ...
        args.v_feat[snapshot_node_index] = v.detach()
    if trainG:
        Generator.train()
        optim_G.zero_grad()
    else:
    ...
    for i, onehots in enumerate(neg_hedge_onehots) :
        ...
        # embedding에 영향을 주는 inv_freq 
        embeddings = torch.matmul(onehots, args.v_feat)
...
def model_eval(args, test_batchloader, g, model, Aggregator,snapshot_node_index):
    model.eval()
    Aggregator.eval()
    with torch.no_grad():
        total_pred = [] 
        ...
        while True :
            ...
            if args.model == "hnhn":
                ...
                e_reg_sum = args.e_reg_sum[g.nodes('edge')]
                v, e = model([g, v_feat, e_feat, v_reg_weight, v_reg_sum, e_reg_weight, e_reg_sum], args.n_layers)
                all_v_feat[snapshot_node_index] = v
            else:
                assert()
                
            for hedge in hedges :
                embeddings = all_v_feat[hedge]
