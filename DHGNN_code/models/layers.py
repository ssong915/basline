...
class DHGLayer(GraphConvolution):
    ...
    def __init__(self, **kwargs):
        ...
        self.train_kmeans = None
        self.val_kmeans = None
        self.test_kmeans = None
        self.train_structure = None
        self.val_structure = None
    ...
    def _structure_select(self, ids, feats, edge_dict, phase):
        """
        ...
        if self.structure is None:
            ...
            if phase == 'train':
                self.train_structure = idx
            elif phase == 'val':
                self.val_structure = idx
            elif phase == 'test':
                self.test_structure = idx
        else:
            if phase == 'train':
                idx = self.train_structure 
            elif phase == 'val':
                idx = self.val_structure
            elif phase == 'test':
                idx = self.test_structure
    ...
    def _cluster_select(self, ids, feats, phase):
        """
        ...
        if self.kmeans is None:
            ...
            for point in range(_N):
                point_cluster_indices = [cluster_center_dict[point][i] for i in range(self.n_center)]
                point_cluster_samples = [sample_ids_v2(point_in_which_cluster[index], self.kc) for index in point_cluster_indices]
                idx.append(point_cluster_samples)
            idx = torch.LongTensor(idx)

            if phase == 'train':
                self.train_kmeans = idx
            elif phase == 'val':
                self.val_kmeans = idx
            elif phase == 'test':
                self.test_kmeans = idx 
        
        else:
            if phase == 'train':
                idx = self.train_kmeans 
            elif phase == 'val':
                idx = self.val_kmeans
            elif phase == 'test':
                idx = self.test_kmeans
    ...
    def forward(self, ids, feats, edge_dict, G, ite, phase):
        hyperedges = []    
        if ite >= self.wu_kmeans:
            if ite == 10:
                self.kmeans = None
            c_feat = self._cluster_select(ids, feats, phase)
        ...
        if ite >= self.wu_struct:
            if ite == 5:
                self.structure = None 
            s_feat = self._structure_select(ids, feats, edge_dict, phase)
        ...
        x = self._edge_conv(x)  
#----------------- [get] updated node feature -----------------------#  # (N, d)
        updated_node_feature = x
        #x = self._fc(x)                                                 # (N, d')
