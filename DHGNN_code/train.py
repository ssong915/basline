...
parser.add_argument('--gpu_id', default='1', help='Visible GPU id')
    ...
    for epoch in range(num_epochs):
        ...
        for phase in ['train', 'val']:
            ...
            with torch.set_grad_enabled(phase == 'train'):
                ...
                # DHGNN output : centroid node가 어떤 category에 속할지에 대한 확률 
                outputs = model(ids=idx, feats=fts, edge_dict=edge_dict, G=G, ite=epo)

#---------------------------- label이 필요한 부분 (node classification task 수행)---------------------#
