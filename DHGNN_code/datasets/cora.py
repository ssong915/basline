...
def load_citation_data(cfg):
    ...
    edge_list = list(G.adjacency())
    #edge_list = list(G.adjacency_list())
    n_edge_list = []

    degree = [0] * len(edge_list)
    if cfg['add_self_loop']:
        for i, (node, neighbors) in enumerate(edge_list):
            # self-loop 추가
            G.add_edge(node, node)
            # 갱신된 인접 리스트 다시 가져오기
            edge_list = list(G.adjacency())
            n_edge_list.append(list(neighbors.keys()))
    ...
    return features, lbls, idx_train, idx_val, idx_test, n_category, edge_list, n_edge_list
