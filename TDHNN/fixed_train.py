import torch
from our_data_load import load_our_data, load_snapshot_data, split_data, load_data
...
def train(model, optimizer, decoder, train_data, all_v_feat, all_sns_neg_hedge, all_cns_neg_hedge, all_mns_neg_hedge, device, args):
    model.to(device)
    decoder.to(device)
    ...
    for epoch in tqdm(range(args.epoch)):
        ...
        for time in range(total_time-1):
            ...
            next_pos_hedge, sns_neg_hedge, cns_neg_hedge, mns_neg_hedge = get_next_samples(train_data[time+1], all_sns_neg_hedge, all_cns_neg_hedge, all_mns_neg_hedge)
            ...
            time_data, time_node = load_data(train_data[time], all_v_feat, device)
            #------------------------ get updated node feature (x) -------------#
            updated_node_feat, H, H_raw = model(time_data, args)
            train_all_v_feat[time_node] = updated_node_feat

            train_all_v_feat = train_all_v_feat.clone().detach()

            train_pos_edges = next_pos_hedge
...
def evaluate(model, test_data, decoder, all_v_feat,
              all_sns_neg_hedge, all_cns_neg_hedge, all_mns_neg_hedge, device, args):
    ...
    total_time = len(test_data)

    for time in range(total_time-1):

        next_pos_hedge, sns_neg_hedge, cns_neg_hedge, mns_neg_hedge = get_next_samples(test_data[time+1], all_sns_neg_hedge, all_cns_neg_hedge, all_mns_neg_hedge)

        time_data, time_node = load_data(test_data[time], all_v_feat, device)

        updated_node_feat, H, H_raw = model(time_data, args)
        test_all_v_feat[time_node] = updated_node_feat

        test_all_v_feat = test_all_v_feat.clone().detach()

        pos_preds = decoder(test_all_v_feat, next_pos_hedge, 'Maxmin')
        pos_preds = pos_preds.squeeze()
        pos_labels = torch.ones_like(pos_preds)

        sns_neg_preds = decoder(test_all_v_feat, sns_neg_hedge, 'Maxmin')
        ...
        mns_neg_preds = decoder(test_all_v_feat, mns_neg_hedge, 'Maxmin')
        ...
        cns_neg_preds = decoder(test_all_v_feat, cns_neg_hedge, 'Maxmin')
        ...
        d = len(next_pos_hedge) // 3
        mixed_neg_hedge = sns_neg_hedge[0:d]+mns_neg_hedge[0:d]+cns_neg_hedge[0:d]
    ...
    print('*' * 20)
    print(f'Test AUROC: {test_roc} Test AP: {test_ap}')
    print('*' * 20)

    return roc_average, ap_average

def train_dhl(device, args):
    ...
    train_dataset, test_dataset, all_dataset = split_data(args.data)
    ...
    best_model, node_feat = train(model, optimizer, decoder, train_dataset, all_v_feat, all_sns_neg_hedge, all_cns_neg_hedge, all_mns_neg_hedge, device, args)

    test_model = model.to(device)
    test_model.load_state_dict(best_model)
    time_auroc, time_ap = evaluate(test_model, test_dataset, decoder, all_v_feat,
            all_sns_neg_hedge, all_cns_neg_hedge, all_mns_neg_hedge, device, args)
        
    return time_auroc, time_ap
