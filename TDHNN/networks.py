...
class HConstructor(nn.Module):
    ...
    def forward(self, inputs, args):
        ...
        for _ in range(self.iters):
            ...
            # 현재 hyperedge 정보와 node들의 정보를 기반으로 계산
            q = self.to_q(inputs)
            k = self.to_k(edges)
            k = F.relu(k)
            v = F.relu(v)
            ...
            # 적절한 hyperedge 개수 계산을 위해, saturation score 계산
...
class HGNN_classifier(nn.Module):
    ...
    def forward(self, data, args):
        ...
        H_raw = []
        for i in range(self.conv_number):
            # convs input : node features
            x, h, h_raw = self.convs[i](x, args)
            # convs output : 새롭게 정의한 hyperedge의 정보를 반영한 node feature
            x = F.relu(x)
            x = F.dropout(x, training=self.training)
            if args.transfer == 1:
