...
class GraphEmbedding(EmbeddingModule):
  ...
  def compute_embedding(self, memory, source_nodes, timestamps, n_layers, n_neighbors=20, time_diffs=None,
    ...
    if n_layers == 0:
      ...
      if edge_idxs.max() >= self.edge_features.size(0):
        print("edge_feature: ", self.edge_features.size(0))
        print("edge_idxs: ",edge_idxs.max())
        raise IndexError("edge_idxs contains an index that is out of bounds.")

      # 데이터 타입이 올바른지 확인
      if edge_idxs.dtype != torch.long:
        edge_idxs = edge_idxs.long()

      # 인덱싱 수행
      try:
        selected_edge_features = self.edge_features[edge_idxs, :]
