...
def split_in_snapshot(args,all_hyperedges,timestamps):
    ...
    if args.time_split_type == 'sec':
        ...
        set_snapshot_data = [item for sublist in snapshot_data[-1] for item in sublist]
        if len(set(set_snapshot_data)) < 4 :
            snapshot_data[-2] = snapshot_data[-2] + snapshot_data[-1]
            snapshot_data.pop()
