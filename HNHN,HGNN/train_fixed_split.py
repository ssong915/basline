...
def model_train(args, train_snapshot_edges, next_snapshot_pos_edges, next_snapshot_neg_edges, st_model, decoder, optimizer, scheduler):  
    ...
    data_dict = data_load.gen_data(args, reindexed_snapshot_edges)
    g = data_load.gen_DGLGraph(reindexed_snapshot_edges).to(device)   
    train_pos_edges, train_neg_edges = utils.get_samples(args.neg_ratio, next_snapshot_pos_edges, next_snapshot_neg_edges, len(train_snapshot_edges))
...
def model_eval(args, dataloader, next_snapshot_pos_edges, next_snapshot_neg_edges, st_model, decoder):
    ...
    while True:
        ...
        valid_pos_edges, valid_neg_edges = utils.get_samples(args.neg_ratio, next_snapshot_pos_edges, next_snapshot_neg_edges, len(val_snapshot_edges))
...
def train(args, st_model, full_data, full_time, decoder, optimizer, scheduler, f_log, j):
    ...
    for epoch in (range(args.epochs)): 
        ...
        for t in tqdm(range(len(snapshot_data)-1)): 
            ...
            train_dataloader = preprocess.BatchDataloader(snapshot_data[t], args.batch_size, device, is_Train=True) 
        ...
        if best_roc < val_auc_roc:
            ...
            best_ap = val_ap                    
            torch.save(st_model.state_dict(), f"{args.model_save}_model_{j}.pkt")
            torch.save(decoder.state_dict(), f"{args.model_save}_decoder_{j}.pkt")
    ...
    st_model.load_state_dict(torch.load(f"{args.model_save}_model_{j}.pkt"))
    decoder.load_state_dict(torch.load(f"{args.model_save}_decoder_{j}.pkt"))
