...
def parse_args():
    ...
    # label    
    parser.add_argument("--neg_ratio", default=1, type=float, help='pos:neg = 1:ratio')
    ...
    # encoder
    parser.add_argument("--model", default='hgnn', type=str, help='encoder: hgnn')
...
def get_samples(ratio ,pos_hyperedges, neg_hyperedges, batch_size):
        
    if ratio > 1:
        neg_size = batch_size
        pos_size = int(math.ceil(batch_size * 0.5))
    else:
        pos_size = batch_size
        neg_size = int(math.ceil(batch_size * ratio))
    
    if len(pos_hyperedges) >= batch_size:
        # 다음 snapshot의 크기가 batch size보다 클 경우 (다음 snapshot에서 뽑을 수 있는 경우)
        # 중복 없이 
        return random.sample(pos_hyperedges, pos_size), random.sample(neg_hyperedges, neg_size)
    else:
        # 다음 snapshot의 크기가 batch size보다 작을 경우
        # 중복 포함
        return random.choices(pos_hyperedges, k=pos_size), random.choices(neg_hyperedges, k=neg_size)
