...
parser.add_argument('-d', '--data', type=str, help='data sources to use, try wikipedia or reddit', default='tags-math-sx')
...
model_path = f'/home/dake/workspace/N_TGAT/Inductive-representation-learning-on-temporal-graphs/saved_models/seed_{args.seed}/{args.prefix}-{args.agg_method}-{args.attn_mode}-{DATA}-{args.n_degree}.pth'
...
for epoch in tqdm(range(args.n_epoch)):
    ...
    for k in tqdm(range(num_batch)):
        ...
        src_feat = np.array(src_embed.cpu())
        dst_feat = np.array(dst_embed.cpu())

        if k==0:
            all_src_feat = src_feat
            all_dst_feat = dst_feat
        else :
            all_src_feat = np.vstack([all_src_feat, src_feat])
            all_dst_feat = np.vstack([all_dst_feat, dst_feat])

np.save('/home/dake/workspace/N_TGAT/Inductive-representation-learning-on-temporal-graphs/final_processed/seed_'+str(r_seed)+'/tgat_'+ DATA +'_'+str(NUM_NEIGHBORS)+ '_embedding_src.npy',all_src_feat)
np.save('/home/dake/workspace/N_TGAT/Inductive-representation-learning-on-temporal-graphs/final_processed/seed_'+str(r_seed)+'/tgat_'+ DATA +'_'+str(NUM_NEIGHBORS)+ '_embedding_dst.npy',all_dst_feat)

print(str(r_seed) +'_'+ str(NUM_NEIGHBORS)+'_'+'success')

print(all_src_feat.shape)
print(all_dst_feat.shape)
