...
class TGAN(torch.nn.Module):
    def __init__(self, ngh_finder, n_feat, e_feat,
        ...
        cls_layers = [self.n_feat_dim, 128, 8, 1]
    ...
    def hyper_contrast(self, src_idx_l, target_idx_l, background_idx_l, cut_time_l, device, num_neighbors=20):
        ...
        target_idx_l = np.array(target_idx_l)
        background_idx_l = np.array(background_idx_l)

        src_embed = src_embed.cpu().detach().numpy()
        target_embed = target_embed.cpu().detach().numpy()
        background_embed = background_embed.cpu().detach().numpy()
        
        all_node_info = np.concatenate([src_idx_l, target_idx_l, background_idx_l])
        all_node_feature = np.concatenate([src_embed, target_embed, background_embed])
        ...
        unique_time_info = np.unique(cut_time_l)
        node_in_pos = {}
        node_in_neg = {}
        index = 0
        ...
        for unit_value in unique_time_info:
            ...
            indices = [i for i, u in enumerate(cut_time_l) if u == unit_value]

            #각 정보에서 같은 hyperedge에 속하는 node들의 정보 모으기
            src_idx = src_idx_l[indices]
            target_idx = target_idx_l[indices]
            background_idx = background_idx_l[indices]

            positive_nodes = np.concatenate([src_idx, target_idx])
            positive_hyperedge = np.unique(positive_nodes)
            negative_nodes = np.concatenate([src_idx, background_idx])
            negative_hyperedge = np.unique(negative_nodes)
        ...
        pos_score = self.hyperedge_score(averaged_features, node_in_pos, 'Maxmin', device)
        neg_score = self.hyperedge_score(averaged_features, node_in_neg, 'Maxmin', device)
